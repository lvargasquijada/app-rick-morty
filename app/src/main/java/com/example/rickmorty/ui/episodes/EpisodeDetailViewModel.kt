...
class EpisodeDetailViewModel : ViewModel() {
    ...
    private val _state = MutableStateFlow<UiState<EpisodeDTO>>(UiState.Loading)

    // StateFlow: "Estado de lectura" -> UI observa
    val state: StateFlow<UiState<EpisodeDTO>> = _state

    fun loadDetail(episodeId: Int) {
        // 1 - Loading
        ...
        viewModelScope.launch {
            ...
            runCatching {
                // 3 - LLamo a la api
                ApiClient.api.getEpisodeById(episodeId)
            }.onSuccess { episodes ->
